...
A front-end colleague needs clear REST endpoints to display notifications when students log in. The platform supports three notification categories: **Placements**, **Events**, and **Results**.
...
Real-time delivery is implemented via **WebSockets** (Socket.IO / native WS).
...
The server authenticates the token, then joins the socket to a room named `student:<studentId>`.

When the backend creates a new notification it emits:
```js
...
The front end listens:
```js
socket.on('notification:new', (notif) => {
  // append to inbox, show toast
});
```

For mass broadcasts (e.g. "Notify All") the server emits to the room `broadcast:all`:
```js
...
Reasons:
- Strong consistency guarantees (ACID) are critical when marking notifications as read or delivering exactly-once semantics.
- Rich index support (partial indexes, composite indexes, GIN for JSONB) covers the query patterns below efficiently.
- Native `ENUM` types match the `notificationType` domain exactly.
- Scales predictably with read replicas and connection pooling (PgBouncer).
...
#### Sample queries based on Stage 1 API
...
1. Add a **partial composite index** covering the exact `WHERE` and `ORDER BY`:
...
This index is small (only unread rows), pre-sorted by `created_at DESC`, and enables an index-only scan. Cost drops from O(N) to O(log N + k) where k = unread count for that student.

2. Replace `SELECT *` with explicit columns to avoid fetching unused data.

**Advice on indexing every column**

Adding an index on every column is counterproductive:
- Each index consumes disk space and must be maintained on every write.
- The query planner may choose a suboptimal index if many are present.
- A standalone boolean index on `is_read` has near-zero selectivity (only 2 values).

Targeted, composite, partial indexes aligned with real query patterns are always preferred.
...
1. On first request for a student's notifications, query PostgreSQL and cache the result in Redis with a 60-second TTL.
...
### Redesigning Bulk Notification Delivery
...
2. **No atomicity** — if `send_email` fails at student 200, the remaining 49,800 get no notification at all.
...
No. They have different failure modes and performance characteristics. The correct pattern:
- **Write to DB atomically first** (single bulk INSERT — source of truth).
- **Enqueue email and push jobs** to a message queue. Workers retry failures independently.
...
With a queue, each email job is independent. If 200 fail they are automatically retried via exponential back-off without affecting the other 49,800. A dead-letter queue captures jobs that exhaust all retries for manual inspection. DB records exist for all students regardless of email delivery status.
...
3. Else if its score > heap minimum, pop the minimum and push the new item.

This keeps the priority inbox updated in **O(log N)** time per notification, regardless of total notification count.

See `notification_app_be/priority_inbox.js` for the complete working implementation.
